...
// Primary LutBuilderHdr shader, used on Uberposts for main scene tonemapping.
...
float4 main(
  ...
  float3 untonemapped = float3(_619, _620, _621);
  ...
  if (_UserLut_Params.w > 0.0f) {
    float _626 = saturate(_619);
    float _627 = saturate(_620);
    float _628 = saturate(_621);
    float _653 = select((_626 <= 0.0031308000907301903f), (_626 * 12.920000076293945f), ((exp2(log2(abs(_626)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    float _654 = select((_627 <= 0.0031308000907301903f), (_627 * 12.920000076293945f), ((exp2(log2(abs(_627)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    float _655 = select((_628 <= 0.0031308000907301903f), (_628 * 12.920000076293945f), ((exp2(log2(abs(_628)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    float _659 = _655 * _UserLut_Params.z;
    float _660 = floor(_659);
    float _666 = ((_654 * _UserLut_Params.z) + 0.5f) * _UserLut_Params.y;
    float _668 = (((_UserLut_Params.z * _653) + 0.5f) * _UserLut_Params.x) + (_660 * _UserLut_Params.y);
    float _669 = _659 - _660;
    float4 _671 = _UserLut.SampleLevel(sampler_UserLut, float2((_668 + _UserLut_Params.y), _666), 0.0f);
    float4 _675 = _UserLut.SampleLevel(sampler_UserLut, float2(_668, _666), 0.0f);
    float _694 = (((_675.x - _653) + ((_671.x - _675.x) * _669)) * _UserLut_Params.w) + _653;
    float _695 = (((_675.y - _654) + ((_671.y - _675.y) * _669)) * _UserLut_Params.w) + _654;
    float _696 = (((_675.z - _655) + ((_671.z - _675.z) * _669)) * _UserLut_Params.w) + _655;
    _725 = select((_694 <= 0.040449999272823334f), (_694 * 0.07739938050508499f), exp2(log2(abs((_694 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
    _726 = select((_695 <= 0.040449999272823334f), (_695 * 0.07739938050508499f), exp2(log2(abs((_695 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
    _727 = select((_696 <= 0.040449999272823334f), (_696 * 0.07739938050508499f), exp2(log2(abs((_696 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  } else {
    _725 = _619;
    _726 = _620;
    _727 = _621;
  }
  float _732 = ((_726 + _725) + _727) * 0.3333333432674408f;
  SV_Target.x = saturate(lerp(_732, _725, _Desaturate));
  SV_Target.y = saturate(lerp(_732, _726, _Desaturate));
  SV_Target.z = saturate(lerp(_732, _727, _Desaturate));
  SV_Target.w = 1.0f;
  return SV_Target;
}
