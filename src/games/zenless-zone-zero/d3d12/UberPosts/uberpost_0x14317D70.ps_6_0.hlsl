...
float4 main(
  ...
  float _566 = tonemapped.x;
  float _567 = tonemapped.y;
  float _568 = tonemapped.z;
  /*
  float _483 = saturate(_476);
  float _484 = saturate(_477);
  float _485 = saturate(_478);
  float _513 = select((_485 <= 0.0031308000907301903f), (_485 * 12.920000076293945f), ((exp2(log2(abs(_485)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z;
  float _514 = floor(_513);
  float _520 = ((select((_484 <= 0.0031308000907301903f), (_484 * 12.920000076293945f), ((exp2(log2(abs(_484)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z) + 0.5f) * _Lut_Params.y;
  float _522 = (((select((_483 <= 0.0031308000907301903f), (_483 * 12.920000076293945f), ((exp2(log2(abs(_483)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z) + 0.5f) * _Lut_Params.x) + (_514 * _Lut_Params.y);
  float _523 = _513 - _514;
  float4 _525 = _InternalLut.SampleLevel(s_linear_clamp_sampler, float2((_522 + _Lut_Params.y), _520), 0.0f);
  float4 _529 = _InternalLut.SampleLevel(s_linear_clamp_sampler, float2(_522, _520), 0.0f);
  float _539 = ((_525.x - _529.x) * _523) + _529.x;
  float _540 = ((_525.y - _529.y) * _523) + _529.y;
  float _541 = ((_525.z - _529.z) * _523) + _529.z;
  float _566 = select((_539 <= 0.040449999272823334f), (_539 * 0.07739938050508499f), exp2(log2(abs((_539 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  float _567 = select((_540 <= 0.040449999272823334f), (_540 * 0.07739938050508499f), exp2(log2(abs((_540 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  float _568 = select((_541 <= 0.040449999272823334f), (_541 * 0.07739938050508499f), exp2(log2(abs((_541 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));*/

  if (UberPostBasePacked1.x > 0.0f) {
    float4 _583 = _Grain_Texture.Sample(s_linear_repeat_sampler, float2(((_Grain_TilingParams.x * TEXCOORD.x) + _Grain_TilingParams.z), ((_Grain_TilingParams.y * TEXCOORD.y) + _Grain_TilingParams.w)));
    float _586 = (_583.w + -0.5f) * 2.0f;
    float _590 = 1.0f - (sqrt(dot(float3(_566, _567, _568), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f))) * UberPostBasePacked1.y);
    _604 = ((((UberPostBasePacked1.x * _566) * _586) * _590) + _566);
    _605 = ((((UberPostBasePacked1.x * _567) * _586) * _590) + _567);
    _606 = ((((UberPostBasePacked1.x * _568) * _586) * _590) + _568);
  } else {
    _604 = _566;
    _605 = _567;
    _606 = _568;
