...
cbuffer $Globals : register(b0) {
  ...
  int _MetalSubpassForCharPPStencil : packoffset(c173.x);
  int _MetalSubpassForCharPPCompFunc : packoffset(c173.y);
  int _MetalSubpassForCharPPReadMask : packoffset(c173.z);
  uint _PunctualLightCount : packoffset(c173.w);
  uint _EnvLightCount : packoffset(c174.x);
  float4 _ShadowBias : packoffset(c175.x);
  float _UseAreaLightMap : packoffset(c176.x);
  float _UseLightSkyOC : packoffset(c176.y);
  float _RimGlowWidthForCharacter : packoffset(c176.z);
  float4 _CloudShadowMoveSpeed : packoffset(c177.x);
  float4 _CloudShadowRotateMatrix[4] : packoffset(c178.x);
  float4 _Lut_Params_Char : packoffset(c182.x);
};
...
float4 main(
  ...
  float _141 = dot(float3(_136, _137, _138), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f)) / dot(float3(_18, _19, _20), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f));
  float _148 = saturate((dot(float3(_9.x, _9.y, _9.z), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f)) + -0.10000000149011612f) * 4.999999523162842f);
  SV_Target.x = (_18 + (_148 * ((_141 * _9.x) - _9.x)));
  SV_Target.y = (_19 + (_148 * ((_141 * _9.y) - _9.y)));
  SV_Target.z = (_20 + (_148 * ((_141 * _9.z) - _9.z)));
