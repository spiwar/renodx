...
float4 main(
  ...
  float _127 = ((mad((_InvViewProjMatrix[2].x), _77.x, mad((_InvViewProjMatrix[1].x), _103, ((_InvViewProjMatrix[0].x) * _101))) + (_InvViewProjMatrix[3].x)) / _119) - _AvatarPosition0.x;
  float _128 = ((mad((_InvViewProjMatrix[2].y), _77.x, mad((_InvViewProjMatrix[1].y), _103, ((_InvViewProjMatrix[0].y) * _101))) + (_InvViewProjMatrix[3].y)) / _119) - _AvatarPosition0.y;
  float _129 = ((mad((_InvViewProjMatrix[2].z), _77.x, mad((_InvViewProjMatrix[1].z), _103, ((_InvViewProjMatrix[0].z) * _101))) + (_InvViewProjMatrix[3].z)) / _119) - _AvatarPosition0.z;
  float _135 = sqrt(((_127 * _127) + (_128 * _128)) + (_129 * _129));
  ...
  float _165 = select((_53.y < _53.z), 0.0f, 1.0f);
  float _170 = (_165 * (_53.y - _53.z)) + _53.z;
  float _171 = (_165 * (_53.z - _53.y)) + _53.y;
  float _173 = 0.6666666865348816f - _165;
  float _175 = select((_53.x < _170), 0.0f, 1.0f);
  float _182 = (_175 * (_53.x - _170)) + _170;
  float _184 = (_175 * (_170 - _53.x)) + _53.x;
  ...
  float _259 = (((_247 * (lerp(_ColorTintNear.x, _ColorTintFar.x, _141))) - _247) * _163) + _247;
  float _260 = (((_248 * (lerp(_ColorTintNear.y, _ColorTintFar.y, _141))) - _248) * _163) + _248;
  float _261 = (((_249 * (lerp(_ColorTintNear.z, _ColorTintFar.z, _141))) - _249) * _163) + _249;
  float _269 = _SliceColorA.w * _52;
  [branch]
  if (_SliceBlendModes.x < 0.5f) {
    _553 = (lerp(_259, _SliceColorA.x, _269));
    _554 = (lerp(_260, _SliceColorA.y, _269));
    _555 = (lerp(_261, _SliceColorA.z, _269));
  } else {
    if (_SliceBlendModes.x < 1.5f) {
      _553 = (_259 + (_269 * _SliceColorA.x));
      _554 = (_260 + (_269 * _SliceColorA.y));
      _555 = (_261 + (_269 * _SliceColorA.z));
    } else {
      if (_SliceBlendModes.x < 2.5f) {
        ...
        float _318 = select((_260 > 1.0f), _260, saturate((exp2(log2(abs(_260)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
        float _319 = select((_261 > 1.0f), _261, saturate((exp2(log2(abs(_261)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
        ...
        if (_SliceBlendModes.x < 3.5f) {
          ...
          float _450 = select((_260 > 1.0f), _260, saturate((exp2(log2(abs(_260)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
          float _451 = select((_261 > 1.0f), _261, saturate((exp2(log2(abs(_261)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
          ...
          _553 = (_259 * ((_269 * (_SliceColorA.x + -1.0f)) + 1.0f));
          _554 = (_260 * ((_269 * (_SliceColorA.y + -1.0f)) + 1.0f));
          _555 = (_261 * ((_269 * (_SliceColorA.z + -1.0f)) + 1.0f));
        }
      }
    }
  }
  float _563 = _SliceColorB.w * (1.0f - _52);
  [branch]
  ...
  if (_TextureSheetParams.w > 0.5f) {
    ...
    float4 _2239 = _VRTextureSheetTex.Sample(s_linear_repeat_sampler, float2(((float((int)(_2228 % _2220)) * (1.0f / float((int)(_2220)))) + (_26 / _TextureSheetParams.x)), ((float((int)(_2228 / _2220)) * (1.0f / float((int)(int(_TextureSheetParams.y))))) + (_47 / _TextureSheetParams.y))));
  ...
  SV_Target.y = _2265;
  SV_Target.z = _2266;
  SV_Target.w = (_2210 + _53.w);
  SV_Target.xyz = renodx::draw::RenderIntermediatePass(renodx::tonemap::UpgradeToneMap(untonemapped.xyz, _53.xyz, SV_Target.xyz));
  return SV_Target;
}
